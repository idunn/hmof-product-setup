		...
		<g:each in="${commerceObjectInstanceList}" status="i" var="commerceObjectInstance">
			...
			File devLogFile = new File(devLog)
			   ...
			   File prodLogFile = new File(prodLog) %>
				...
				<td><sec:ifAnyGranted roles="ROLE_PM, ROLE_QA, ROLE_PROD"><input type="radio" name="rad" id="rad${i}" value="${commerceObjectInstance.id+"/"+jobdetails.getCurrentEnversRevision(commerceObjectInstance)+"/"+jobdetails.getPromotionDetails(commerceObjectInstance,jobdetails.getUserEnvironmentInformation())+"/false"}"
